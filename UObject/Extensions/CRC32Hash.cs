...
namespace UObject.Extensions
{
    ...
    {
        private static readonly uint[] TableCCITT =
        {
        ...
        private static readonly uint[] TableIEEE802 =
        {
        ...
        // GetNCPHash
        public static ushort GetCCITTHash(ref string name)
        {
            var hash = 0u;
            var utf16 = IsWide(ref name);
            foreach (var t in name)
            {
                if (!utf16)
                {
                    var value = Convert.ToByte(char.ToUpper(t));
                    hash = hash >> 8 & 0x00FFFFFF ^ TableCCITT[(hash ^ value) & 0x000000FF];
                }
                else
                {
                    var chr = t;
                    var value = Convert.ToUInt16(char.ToUpper(chr));
                    hash = hash >> 8 & 0x00FFFFFF ^ TableCCITT[(hash ^ value) & 0x000000FF];
                    value = Convert.ToUInt16(chr >> 8);
                    hash = hash >> 8 & 0x00FFFFFF ^ TableCCITT[(hash ^ value) & 0x000000FF];
                }
            }
        ...
        internal static ushort GetIEEEHash(ref string name)
        {
            ...
            if (!IsWide(ref name))
            {
                ...
                {
                    hash = hash >> 8 ^ TableIEEE802[t ^ hash & 0xFF];
                    for (var j = 0; j < 3; j++)
            ...
            else
            {
                ...
                {
                    var chr = t;
                    hash = hash >> 8 ^ TableIEEE802[(hash ^ chr) & 0xFF];
                    for (var j = 0; j < 3; j++)
                        hash = hash >> 8 ^ TableIEEE802[(hash ^ (chr >>= 8)) & 0xFF];
                }
            }
        ...
        private static bool IsWide(ref string text)
        {
